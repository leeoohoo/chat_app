...
const ApplicationsPanel: React.FC<ApplicationsPanelProps> = ({ isOpen, onClose, manageOnly = false, title, layout = 'modal' }) => {
  ...
  const openAppWindow = (app: any) => {
    ...
    if (existingWindow && !existingWindow.closed) {
      ...
      const popupWidth = Math.min(900, Math.floor(screenWidth * 0.4));
      const popupHeight = screenHeight - 100;
      const popupLeft = 0;
      const popupTop = 50;

      const features = `width=${popupWidth},height=${popupHeight},left=${popupLeft},top=${popupTop},toolbar=no,menubar=no,location=yes,status=yes,resizable=yes,scrollbars=yes`;
